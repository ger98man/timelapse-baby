...
self.addEventListener('fetch', e => {
  ...
  e.respondWith((async () => {
    try {
      // no-cache — не «не кэшировать», а «сверить с сервером». Без этого
      // «сеть вперёд» получается только на словах: обычный fetch спрашивает
      // сначала кэш браузера, и файл без Cache-Control (а локальный
      // python -m http.server других не отдаёт) залипает там по эвристике
      // на часы. Приезжает старый стиль, и он же ложится в оболочку.
      // С проверкой невыгодного нет: не менялся — сервер ответит 304.
